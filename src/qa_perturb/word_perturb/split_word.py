    ...
    def split_word(self):
        ...
        for index, sample in enumerate(tqdm(self.data)):
            ...
            if self.pos_tag is None: # get random words to perturb
                random_words = get_word(words, self.max_words, self.length_of_word_to_perturb, self.data_field, sample['answers']) 
                self.name = 'split_random_word'
            elif self.pos_tag.lower() == 'verb': # get random verbs to perturb
                random_words = get_verb(words, self.max_words, self.length_of_word_to_perturb, self.data_field, sample['answers'])
